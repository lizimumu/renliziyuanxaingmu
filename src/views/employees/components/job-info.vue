<template>
  <div class="job-info">
    ...
    <i class="el-icon-printer" @click="$router.push('/employees/print/'+userId +'?type=jop')" />
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveEmployeesJop">保存更新</el-button>
...
import { getEmployeesJopInfo, saveEmployeesJopInfo } from '@/api/employees'
export default {
  data() {
    return {
  ...
  created() {
    this.loadEmployeesTob()
  },
  methods: {
    async  loadEmployeesTob() {
      this.formData = await getEmployeesJopInfo(this.userId)
    },
    async   saveEmployeesJop() {
      await saveEmployeesJopInfo(this.formData)
